...
echo "Headliner's name will now be part of folder name"
echo "Logs are no longer optional"
echo
echo "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-"
echo
...
echo
echo
read -r -p "would you like to start this backup?  [y/N] " lresponse
case "$lresponse" in
    [yY][eE][sS]|[yY]) 
    echo "GREAT SHOW!!!"
    echo
...
folderhash=$(gdrive mkdir "$headliner shows "$(date +"%m-%d-%Y") -p 1Cd9hwC_NwxYZBQYjImbnV30cUrAEp8xS | grep Directory | awk {'print $2'})
echo
echo
echo
